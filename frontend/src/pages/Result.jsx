    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <section className="max-w-[1100px] mx-auto px-6 lg:px-12 py-12 lg:py-16">
        <div className="no-print flex items-center justify-between mb-8">
          {isShareMode ? (
            <Link to="/" className="inline-flex items-center gap-2 text-sm text-ink/60 hover:text-ink" data-testid="result-back">
              <ArrowLeft size={16} /> Retour à l'accueil
            </Link>
          ) : (
            <Link to="/builder" className="inline-flex items-center gap-2 text-sm text-ink/60 hover:text-ink" data-testid="result-back">
              <ArrowLeft size={16} /> Modifier ma lettre
            </Link>
          )}
          {!isShareMode && c && (
            <p className="eyebrow text-ink/60 hidden sm:block">{c.title}</p>
          )}
          {isShareMode && (
            <p className="eyebrow text-ink/60 hidden sm:block" data-testid="share-mode-badge">Lettre partagée — lecture seule</p>
          )}
        </div>

        <div className="no-print">
          <p className="eyebrow text-ink/60">Votre lettre</p>
          <h1
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-10">
          ...
          <div className="lg:col-span-8">
            {!isShareMode && (
              <div className="no-print flex items-center gap-2 mb-3">
                ...
                {isEdited && (
                  <span className="text-xs text-sage font-medium" data-testid="edited-badge">
                    • Texte modifié
                  </span>
                )}
              </div>
            ...
            >
              ...
              {!isShareMode && editing ? (
                <textarea
                  ...
                  className="w-full p-8 sm:p-12 font-mono-letter text-[13px] sm:text-[14px] leading-[1.7] text-ink bg-white outline-none resize-none border-0 focus:ring-0"
                  style={{ minHeight: "70vh" }}
                  data-testid="result-editor"
                  spellCheck={true}
                />
              ) : (
                <pre className="p-8 sm:p-12 font-mono-letter text-[13px] sm:text-[14px] leading-[1.7] text-ink whitespace-pre-wrap">
                  {baseText}
                </pre>
              )}
            </div>
          ...
          <aside className="no-print lg:col-span-4 space-y-3">
            <div className="rounded-md border border-ink/15 bg-card p-6">
              <p className="font-serif-display text-xl">Actions</p>
              <p className="text-sm text-ink/60 mt-1">Téléchargez, copiez ou partagez votre lettre.</p>

              <div className="mt-5 space-y-2.5">
                <button
                  type="button"
                  onClick={handlePDF}
                  className="btn-amber w-full inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-full text-sm font-semibold"
                ...
                <button
                  type="button"
                  onClick={handleCopy}
                  className="w-full inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-full text-sm font-medium border border-ink/20 hover:border-ink transition"
                ...
                {!isShareMode && (
                  ...
                  >
                    {linkCopied ? <Check size={16} className="text-sage" /> : <Share2 size={16} />}
            ...
            {/* LRAR — Et maintenant ? */}
            {!isShareMode && (
              <div className="rounded-md border border-ink/15 bg-card p-6" data-testid="lrar-card">
                <p className="eyebrow text-ink/50">Optionnel</p>
                <p className="font-serif-display text-xl mt-2 leading-tight">
                ...
                <a
                  ...
                  className="mt-4 inline-flex items-center gap-2 px-5 py-3 rounded-full text-sm font-medium border border-ink hover:bg-ink hover:text-paper transition"
            ...
            <div className="rounded-md border border-ink/15 bg-paper-2 p-6">
              <div className="flex items-start gap-3">
                <Lock size={18} className="text-sage shrink-0 mt-0.5" />
                <div>
            ...
            {!isShareMode && (
              <div className="rounded-md border border-ink/15 bg-paper-2 p-6">
                <div className="flex items-start gap-3">
                  <FileText size={18} className="text-sage shrink-0 mt-0.5" />
                  <div>
